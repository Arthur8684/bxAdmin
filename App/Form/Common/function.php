...
function GPS($IP){
	$data = @file_get_contents("http://api.map.baidu.com/location/ip?ak=FobcQnOlVGViquHzR5mr6iQr&ip=".$IP."&coor=bd09ll");
	$arr = json_decode($data,true);
	return $arr;
 }

function get_Location($x,$y){
	$data = file_get_contents("http://api.map.baidu.com/geocoder/v2/?ak=FobcQnOlVGViquHzR5mr6iQr&callback=renderReverse&location=".$x.",".$y."&output=json&pois=1");
	$data = substr($data,29);
