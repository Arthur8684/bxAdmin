...
*/
 function pay_cannal_pay($id,$coin_type='money')
 {
	 ...
	 if($order_info){
		...
		if(account($order_info['user_id'],array($coin_type=>$order_amount),1,5,'SYSTEM','订单'.$order_info['order_sn'].'退款')){
...
用户信息
*/
 function user_info($id,$feild='')
 {
	$user=M('user');
	$where['id']=$id;
	if($feild){
		$info=$user->where($where)->getField($feild);
	}else{
...
*/
 function order_status($id,$status,$name,$air=1,$pay_type=0,$is_pay=1,$coin_type='money')
 {
	...
	if($name=='pay_status' && $status==1){
		...
		$data['pay_time']=time();
		//写入资金
		if(account($user_id,array($coin_type=>-$order_amount),$operation_type=1,$pay_type,'SYSTEM',$msg='订单'.$order_sn.'购买',$is_pay)){
		   $st=array('status'=>'SUCCES');
		   }else{
	...
	if($name=='ship_status' && $status==2){
		...
		if($shop_id){
		if(account($shop_id,array($coin_type=>$order_amount),1,$pay_type,'SYSTEM','订单'.$order_sn.'消费',$is_pay)){
			//地区分销
			 load("User/function");
...
/*订单状态显示*/
 function order_status_html($id)
 {
	...
	if($orderinfo){
		$html.="<span style='color:".order_status_color($orderinfo["order_status"])."'>".L('order_status_'.$orderinfo["order_status"].'')."</span> ";
		$html.="<span style='color:".order_status_color($orderinfo["pay_status"])."'>".L('pay_status_'.$orderinfo["pay_status"].'')."</span> ";
		$html.="<span style='color:".order_status_color($orderinfo["ship_status"])."'>".L('ship_status_'.$orderinfo["ship_status"].'')."</span>";
	}
	return $html;
 }
/*订单状态字体颜色*/
...
/*
添加购物车
$type 0为常规添加 1为仅添加这一次并返回添加id
*/

 function add_cart($goods_id,$user_id,$goods_num,$model_id,$type=0)
 {	
	...
	if($type==1){
		...
		$shop=M('shop');
		$shop_info=$shop->where('user_id='.$goods['autho_id'].'')->find();
		if($shop_info) $data['shop_id']=$shop_info['id'];
		return $cart->add($data);
	}		
 }		 
/*
...
$discount 折扣
*/
 function settle_cart($cart_separate,$userid,$consignee_id,$discount=1,$order_come=0,$coin_type='money')
 {
	$order=M('order');
	$order_info=M('order_info');
	foreach($cart_separate['goods_info'] as $key=>$v){
		$nowtime=time();	
		//生成收货信息
		$consignee_info=consignee_info($consignee_id);
		//生成订单号
		$u=rand();
		...
		//添加到订单信息中
			foreach($v['info'] as $val){
				...
				$price=goods_price($val['goods_id'],$val['model_id'],$val['shop_id']);
				$data['amount']=$price*$val['goods_num'];
				$data['model_id']=$val['model_id'];		
				$data['shop_id']=$val['shop_id'];
				$order_info->add($data);
				$order_price=$order_price+$price*$val['goods_num'];				
			 ...
			 $orderdata['order_come']=$order_come;	
			 $orderdata['coin_type']=$coin_type;	
			 ...
			 foreach($goodname as $val){
				 ...
				 $goodnames = substr($goodnames,0,strlen($goodnames)-1); 
/*				load('Wechat/function');
	  			$cofig=FF('wechat/wechat_user');
	 ...
	 $order_cn_html = substr($order_cn_html,0,strlen($order_cn_html)-1); 
	 return $order_cn_html;
	 }
/*
获取订单商品价格
*/
	function goods_price($goods_id,$model_id,$shop_id){
		  $goods=goods_info($goods_id,$model_id);
		  $price=$goods['price'];
		  $shop_price=M('goods_property')->where(array('goods_id'=>$goods_id,'model_id'=>$model_id,'shop_id'=>$shop_id))->getfield('price');
		  if($shop_price>0){
			$price=$shop_price;  
			  }
		  return $price;
...
/*核销兑换码*/
 function verification_key($conversion_key,$user_id=0,$oprater_shop_shop=0,$coin_type='money')
 {		
 	 $order_conversion_key=M('order_conversion_key');
	 $order_info=M('order_info');
	 $order=M('order');
	 ...
	 if($oprater_shop_shop==0){
	 	 account($shop_info["user_id"],array($coin_type=>$zz_all),1,6,'来自订单'.$order_sn.'销码消费');	
	 }else{
		 $data['oprater_shop_shop']=$oprater_shop_shop;
		 }
	 //区域分销
	 load("User/function");
	 ...
	 //三级分销
	 load("Fenxiao/function");
	 rebate_recommend($order_s["user_id"],$proportion_zz,'来自订单'.$order_sn.'销码消费');
     $arrays=array('order_amount'=>$zz,'goods_id'=>$order_info_info['goods_id'],'model_id'=>$order_info_info['model_id'],'user_id'=>$u_user_id,'order_sn'=>''.$order_sn.'');
	 $order_conversion_key->where($where)->save($data);
	 if($map2['id']){
	 $order_count2=$order_conversion_key->where($map2)->count();
	 }
	 if($order_count2==0){
		 $order_info=order_info($order_sn);
		 $where_up['order_sn']=$order_sn;
		 $data_up['shipping_status']=2;
		 $order->where($where_up)->data($data_up);
		 }
		return $arrays;
	 }
	 }else{
...
//通过店铺id算出分成比例
  function shop_proportion($shop_id)
  {
	  $shop=M('shop');
	  $where["id"]=$shop_id;	
	  $shop_info=$shop->where($where)->find();
	  $proportion=isset($shop_info['proportion'])?$shop_info['proportion']:0;
	  $proportion=$proportion*0.01;
	  return $proportion;
  }
		
function link_list_info_1($parentid,$tables){
	$table=M($tables);
	...
	foreach($link_list as $val){
		$vals["n"]=$val["name"];
		$vals["v"]=$val["id"];
		$vals["s"]=link_list_info_1($val["id"],$tables);
...
-----------------------------------  
* 根据订单id获取订单号减对应产品库存
$order int string 订单号或者订单ID
$inventory_type int 减库存的方式，0表示直接删除产品库存，1表示删除店铺库存
-----------------------------------   
*/	
function order_minus_inventory($order,$inventory_type=1)
{ 
       $m=M('order');
	   if(is_numeric($order)) $order=$m->where(array('id'=>$order))->getField('order_sn');
	   if(!$order) return false;
	   $order_info=M('order_info')->where(array('order_sn'=>$order))->select();   
	   $goods_m=M();
       $goods_m->startTrans();//事务开始
	   $return=true;
	   foreach($order_info as $k=>$v)
	   {
		   if($v)
		   {
                if($inventory_type==1)
				{
					 $return==M('goods_property')->where(array('goods_id'=>$v['goods_id'],'model_id'=>$v['model_id'],'shop_id'=>$v['shop_id']))->setDec('inventory',$v['order_num']);
					 $quantity=M('goods_property')->where(array('goods_id'=>$v['goods_id'],'model_id'=>$v['model_id'],'shop_id'=>$v['shop_id']))->getField('inventory');
					 M('inventory_shop')->data(array('shop_id'=>$v['shop_id'],'model_id'=>$v['model_id'],'goods_id'=>$v['goods_id'],'quantity'=>$quantity,'inventory'=>-$v['order_num'],'addtime'=>time(),'msg'=>'订单【'.$order.'】购买'))->add();
			    }
				else
				{
					 $return=M(model_f($v['model_id']))->where(array('goods_id'=>$v['goods_id']))->setDec('inventory',$v['order_num']);
					 $quantity=M(model_f($v['model_id']))->where(array('goods_id'=>$v['goods_id']))->getField('inventory');
					 M('inventory_shop')->data(array('shop_id'=>0,'model_id'=>$v['model_id'],'goods_id'=>$v['goods_id'],'quantity'=>$quantity,'inventory'=>-$v['order_num'],'addtime'=>time(),'msg'=>'来自订单【'.$order.'】购买'))->add();
				}
				if(!$return) 
				{
					$return_data=array('goods_id'=>$v['goods_id'],'model_id'=>$v['model_id'],'shop_id'=>$v['shop_id']);
				    break;
				}
		   }
	   }
	   ...
	   if($return)
	   {
		    $goods_m->commit();//成功则提交
			$return_data['code']=0;
	   }
	   else
	   {
		    $goods_m->rollback();//不成功，则回滚
			$return_data['code']=1;
	   }
