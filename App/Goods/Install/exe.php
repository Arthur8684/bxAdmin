...
{
	$data=array('name'=>'商城','type'=>'Goods','setting'=>'','model_class'=>'content');
    M('sys_model')->add($data);
};

$this->install_fail=function($sign)
{
	M('sys_mode')->where(array('type'=>'Goods'))->delete();
};

$this->uninstall_before=function($sign)
{
	$model_info=M('model')->field('id,table')->where(array('type'=>'Goods','model_class'=>'content'))->select();
	...
	M('sys_model')->where(array('type'=>'Goods'))->delete();// 系统模型
};
