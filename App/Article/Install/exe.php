...
{
	$data=array('name'=>'文章','type'=>'Article','setting'=>'','model_class'=>'content');
    M('sys_model')->add($data);
};

$this->install_fail=function($sign)
{
	M('sys_mode')->where(array('type'=>'Article'))->delete();
};

$this->uninstall_before=function($sign)
{
	$model_info=M('model')->field('id,table')->where(array('type'=>'Article','model_class'=>'content','table'=>array('not in',array('wechat','notice','chat_article') )))->select();
	...
	M('sys_model')->where(array('type'=>'Article'))->delete();// 系统模型
};
