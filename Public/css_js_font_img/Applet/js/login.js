$(function(){
	...
	if (third == 1) {
	    ...
	    setTimeout(function(){
	    		 $('.w_phone').parent('.hide').removeClass('hide');
	    },500)
	     $('.bound_text,nav,.long-line,.w_phone').show();
   }
	...
	$('#to-reg').on('click', function(){
		...
		// $('.w_enter').hide();
		$('.w_phone,#login_in,.w_enter,.bound_text').hide();
	  $('.w_register,#login_enroll,.w_register,#foot_bottom,#register-nav,.login_reset,nav,.long-line').show();
  ...
  $('#login_enroll').on('click', function(event) {
	...
	$('#forget-PW').on('click', function(){
		...
		$('.w_register,#register-nav,.login_tab,.find-password,#foot_bottom,nav,.long-line,.w_pw').show();
		$('.verify_login,.reset_login,.verify_text,.w_register,.existing_account,.w_enter').hide();
  ...
  $('.pw-S-F').on('click', function(event) {
	...
	$('#to-login').on('click', function(){
		...
		$('.w_enter,#foot_bottom,.fill_in_login').show();
...
$('.login_corner_mark').mouseover(function(event) {
	// if($('#register_pw').val() == ('type','text')){
	// 	$("#register_pw").attr('type','password');
	// }else($('#register_pw').val() == ('type','password')){
...
function verifyEmailCode() {
 ...
 //       $('.login_phone select').css({background: 'url(//testfe.zhichiwangluo.com/static/login/images/login_phone.png) no-repeat','background-position': '6px 8px'});
 //       $('.email_text').hide();
	// 	}else if(index=='1'){
	// 		$('.login_phone input').attr("placeholder","请输入邮箱");
	// 		$('.login_phone select').css({background: 'url(//testfe.zhichiwangluo.com/static/login/images/login_mail.png) no-repeat','background-position': '6px 8px'});
	// 		$('.email_text').show();
	// 	}
	// })
	// 清空input
  ...
  function cometRequest(){
	...
	function cookieLogin(){
		var username = getCookie("pc_un"),
			...
			url = ['/index.php?r=Login/login','/index.php?r=Login/LoginCompany'];
		if (!username || !password  || $("#automatic_login").attr("is_login") == 2) {
			return false;
		}
		$.ajax({
			url:'/index.php?r=Login/login',
