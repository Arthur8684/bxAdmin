...
! function(a) {
	...
	a.laydate = function(b) {
		...
		if (c.ymd[1] + 1 == c.mins[1] && c.ymd[2] == c.mins[2]) {
			...
			if (1 === b && (c.hmsin[0].value < c.mins[3] || (c.hmsin[0].value == c.mins[3] && a < c.mins[4]))) return 1;
			if (2 === b && (c.hmsin[0].value < c.mins[3] || (c.hmsin[0].value == c.mins[3] && c.hmsin[1].value < c.mins[4]) || (c.hmsin[0].value == c.mins[3] && c.hmsin[1].value == c.mins[4] && a < c.mins[5]))) return 1
		...
		c.on(b(e.box), "click", function() {
			...
			c.each(e, function(b, e) {
				f ? c.addClass(e, h[1]) : c.timeVoid(b, d) ? c.addClass(e, h[1]) : c.on(e, "click", function(m) {
					c.stopmp(m).reshow(), c.hasClass(this, h[1]) || (a.value = c.digit(0 | this.innerHTML))
				})
    ...
    var a = c.query("#laydate_box");
    if(a && (a.style.display == '' || a.style.display == 'block')){
      c.close();
    }
	}
}(window);
